...
from armada_backend.logic.utils import get_ship_name, get_ship_ip
...
class Info(object):
    def on_get(self, req, resp):
        try:
            ...
            message = 'Could not get armada info.'
            resp.body = json.dumps({'error': "API exception: {0}. {1} - {2}".format(message, type(e).__name__, str(e)),
                                    'status': 'error'})
        else:
            resp.context['data'] = {'result': result}
